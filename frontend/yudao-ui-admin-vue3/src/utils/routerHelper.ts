...
const isTenantMenuRoute = (route: AppCustomRouteRecordRaw) => {
  ...
  return (
    name.includes('租户') ||
    path.includes('/system/tenant') ||
    path.includes('tenant-package') ||
    component.includes('/system/tenant') ||
    component.includes('tenantPackage') ||
    componentName.includes('Tenant')
  )
}
/**
 * 注册一个异步组件
...
export const generateRoute = (routes: AppCustomRouteRecordRaw[]): AppRouteRecordRaw[] => {
  ...
  for (const route of routes) {
    if (!tenantEnable && isTenantMenuRoute(route)) {
      continue
    }
    // 1. 生成 meta 菜单元数据
    const meta = {
